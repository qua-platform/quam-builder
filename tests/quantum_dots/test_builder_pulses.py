...
class TestAddDefaultLDVQubitPulses:
    ...
    def test_add_xy_pulses_to_qubit_with_xy(self):
        ...
        for pulse_name in expected_pulses:
            assert pulse_name in qubit.xy.operations, f"Pulse {pulse_name} not found"
            assert qubit.xy.operations[pulse_name] is not None
    ...
    def test_add_pulses_to_qubit_with_both_xy_and_resonator(self):
        ...
        assert len(qubit.xy.operations) == 6  # 6 XY pulses
        assert "readout" in qubit.resonator.operations
