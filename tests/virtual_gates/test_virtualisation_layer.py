...
def test_resolve_voltages_simple_1_to_1():
    ...
    input_voltages = {"v_g1": 1.0, "P1": 0.0}  # P1 needs to be in input for +=
...
def test_resolve_voltages_2_to_2():
    ...
    input_voltages = {"v_s1": 2.0, "v_s2": 1.0, "P1": 0.0, "P2": 0.0}
    resolved = vl.resolve_voltages(input_voltages, allow_extra_entries=True)
    ...
    assert np.isclose(resolved["P1"], 0.5)
...
def test_resolve_voltages_partial_source_gates_input():
    ...
    assert np.isclose(resolved["P1"], 1.0)  # P1 = 0.0 + 1.0 * 1.0 (from v_s1)
    assert np.isclose(
        resolved["P2"], 0.0
...
def test_to_dict_conversion():
    ...
    vl_dict = vl.to_dict()
    assert isinstance(vl_dict["matrix"], list)
    assert vl_dict["matrix"] == [[2.0]]
