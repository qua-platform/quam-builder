...
class TestXYDriveSingleCreation:
    ...
    def test_no_default_pulses_on_creation(self):
        ...
        assert "gaussian_x90" not in drive.operations
    ...
    def test_add_custom_pulse(self):
        ...
        drive.add_pulse(
            "rabi", pulses.GaussianPulse(length=100, amplitude=0.2, sigma=20)
        )
...
class TestXYDriveSingleInQUA:
    def test_play_pulse_compiles(self):
        ...
        drive.operations["gaussian_x90"] = pulses.GaussianPulse(
            length=100, amplitude=0.2, sigma=40
        )
        with qua.program() as prog:
            drive.play("gaussian_x90")
    ...
    def test_play_with_duration_override(self):
        ...
        with qua.program() as prog:
            t = qua.declare(int)
            qua.assign(t, 100)
            drive.play("gaussian_x90", duration=t)
        assert prog is not None
