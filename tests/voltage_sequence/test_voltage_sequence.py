...
def test_step_to_voltages_single_channel(machine):
    ...
    with qua.program() as prog:
        seq = machine.gate_set.new_sequence()
        seq.step_to_voltages(levels={"ch1": 0.25}, duration=120)
    ast = ProgramTreeBuilder().build(prog)
...
def test_step_to_voltages_multiple_channels(machine):
    ...
    with qua.program() as prog:
        seq = machine.gate_set.new_sequence()
        seq.step_to_voltages(levels={"ch1": 0.15, "ch2": -0.1}, duration=160)
    ast = ProgramTreeBuilder().build(prog)
...
def test_step_to_voltages_then_step_to_point(machine):
    ...
    with qua.program() as prog:
        seq = machine.gate_set.new_sequence()
        seq.step_to_voltages(levels={"ch1": 0.1}, duration=100)
...
def test_sequence_with_qua_variable_duration_step_to_voltages(machine):
    ...
    with qua.program() as prog:
        ...
        seq.step_to_voltages(levels={"ch1": 0.2}, duration=qua_duration)
    ast = ProgramTreeBuilder().build(prog)
...
def test_sequence_with_qua_variable_voltage_step_to_voltages(machine):
    ...
    with qua.program() as prog:
        ...
        seq.step_to_voltages(levels={"ch1": qua_voltage, "ch2": 0.1}, duration=100)
    ast = ProgramTreeBuilder().build(prog)
...
def test_ramp_to_zero_immediate(machine):
    ...
    with qua.program() as prog:
        seq = machine.gate_set.new_sequence()
        seq.step_to_voltages(levels={"ch1": 0.2, "ch2": -0.15}, duration=100)
...
def test_ramp_to_zero_with_duration(machine):
    ...
    with qua.program() as prog:
        seq = machine.gate_set.new_sequence()
        seq.step_to_voltages(levels={"ch1": 0.25}, duration=100)
...
def test_ramp_to_zero_with_duration_multiple_channels(machine):
    ...
    with qua.program() as prog:
        seq = machine.gate_set.new_sequence()
        seq.step_to_voltages(levels={"ch1": 0.25}, duration=100)
        seq.step_to_voltages(levels={"ch1": 0.25, "ch2": 0.25}, duration=100)
        seq.ramp_to_zero(ramp_duration=200)
    ast = ProgramTreeBuilder().build(prog)
...
#     with qua.program() as prog:
#         seq = machine.gate_set.new_sequence()
#         seq.step_to_voltages(levels={"ch1": 0.1}, duration=1000) # Positive integrated_v
#         seq.apply_compensation_pulse(max_voltage=0.4)
#     # ast = ProgramTreeBuilder().build(prog)
#     # Expected AST: step_to_voltages, then a negative pulse on ch1 for compensation.
...
#         seq.step_to_voltages(levels={"ch1": qua_level}, duration=qua_dur)
#         seq.apply_compensation_pulse(max_voltage=0.4)
#     # ast = ProgramTreeBuilder().build(prog)
#     # Expected AST: QUA assignments, step_to_voltages, then QUA logic for compensation.
