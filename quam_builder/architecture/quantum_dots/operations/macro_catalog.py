...
class VoltageBalancedMacroCatalog:
    """Macro catalog that registers voltage-balanced implementations.
    ...
    * :class:`~quam_builder.architecture.quantum_dots.components.sensor_dot.SensorDot` -- ``measure``
      → :class:`~quam_builder.architecture.quantum_dots.operations.voltage_balanced_macros.state_macros.BalancedSensorDotMeasureMacro`

    Tuning points must follow the docstrings of those classes (e.g. positive
    ``empty`` / ``measure`` / ``exchange`` entries; ``BalancedXYDriveMacro``
    assumes 0 V idle on all gate channels).
    """
    ...
    def get_factories(self, component_type: type) -> MacroFactoryMap:
        ...
        if component_type is QuantumDotPair:
            return {
                ...
                VoltagePointName.EMPTY.value: BalancedEmptyMacro,
                VoltagePointName.MEASURE.value: BalancedMeasurePSBPairMacro,
            }
        if component_type is SensorDot:
            return {VoltagePointName.MEASURE: BalancedSensorDotMeasureMacro}
        return {}
