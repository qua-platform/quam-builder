...
class ReadoutResonatorIQ(InOutIQChannel, ReadoutResonatorBase):
    ...
    def __post_init__(self):
        if hasattr(self.opx_output_I, "upsampling_mode"):
            self.opx_output_I.upsampling_mode = "mw"
        if hasattr(self.opx_output_Q, "upsampling_mode"):
            self.opx_output_Q.upsampling_mode = "mw"
