...
DEFAULT_PULSE_NAME = "step"
DEFAULT_WF_AMPLITUDE = 0.1

DEFAULT_PULSE_NAME_QDAC = 'step'
DEFAULT_PULSE_AMPLITUDE_QDAC = 0.1
...
class VoltageSequence:
    ...
    def _play_ramp_on_channel(
        ...
        if is_qua_type(delta_v) or is_qua_type(ramp_duration_ns):
            ...
            assign(ramp_rate, delta_v * Math.div(1, ramp_duration_ns))
            channel.play(
                ramp(ramp_rate),
...
class QdacVoltageSequence(VoltageSequence):
    def __init__(self, gateset: QdacGateSet, track_integrated_voltage = False):
        super().__init__(gateset, track_integrated_voltage)
        self.gateset = gateset

    def ramp_to_level_with_QDAC(self, virt_levels: Dict[str,float], duration:int, ramp_duration: int, dc_threshold: float = 0.05):

        absolute_physical_gates = self.gateset.resolve_voltages(virt_levels)
        dc_now = {ch: self.gateset.channels[ch].get_dc_V() for ch in self.gateset.channels}
        opx_now = {ch: self.state_trackers[ch].current_level for ch in self.state_trackers}
        opx_changes = {ch: absolute_physical_gates[ch]-dc_now[ch] for ch in absolute_physical_gates}

        if any(value>dc_threshold for value in opx_changes.values()):
            raise ValueError('OPX span too big; change QDAC value or lower your threshold')
        new_opx_vals = {ch: opx_now[ch] + (absolute_physical_gates[ch]-dc_now[ch]) for ch in absolute_physical_gates}

        self.ramp_to_level(new_opx_vals, ramp_duration = ramp_duration, duration = duration)
...
class QdacVoltageSequence:

    def __init__(self, gateset: VirtualQdacGateSet, track_integrated_voltage: bool = False):
        self.gateset = gateset
        self.points: List[VoltageTuningPoint] = []

    def _length_checker(self, amps: Dict[str,float], max_channels: int = 8, threshold:float = 50e-6):
            """
            Iteratively check for the number of simultaneously played pulses, and ensure that this does not exceed the number of cores
            If the number of cores is exceeded, the programme will zero out the smallest pulses
            """
            non_zeros = [k for k, v in amps.items() if abs(v)>0]

            while len(non_zeros)>max_channels:
                warnings.warn(f'{len(non_zeros)} Pulses but only {max_channels} Cores available; zeroing all pulses |v|<({threshold})')
                small_pulses = [k for k, v in amps.items() if abs(v) < threshold]
                for key in small_pulses:
                    amps[key] = 0
                non_zeros = [k for k, v in amps.items() if abs(v)>0]
                threshold = threshold*2

    def generate_qua_program(self, pulse_amplitudes: Dict[str, Dict[str, float]], pulse_durations: Dict[str, int], repetitions: int):
        """Generates the QUA programme based on the pulse_sequence_builder dictionary of pulses and their amplitudes.
        - Aligns up to 8 pulses; when specifying pre-set points, make sure up to 8 gates are pulsed at one time, since OPX has 8 cores
        - Make sure that each of the SingleChannel elements has a square pulse able to play
        """

        channels = self.gateset.channels
        readout_channel = self.gateset.readout
        
        for pulse in pulse_amplitudes.values():
            self._length_checker(amps = pulse, max_channels = 8, threshold = 50e-6)
        ...
        with program() as prog: 
            ...
            with for_(n, 0, n < repetitions, n+1):
                for pulse_key in sorted(pulse_amplitudes.keys()):
                    amps, duration = pulse_amplitudes[pulse_key], pulse_durations[pulse_key]

                    for gate_name, pulse_height in amps.items():
                        if pulse_height == 0:
                            continue
                        ch = channels[gate_name]
                        amp_scale = pulse_height / ch.operations[DEFAULT_PULSE_NAME_QDAC].amplitude
                        ch.play(
                            DEFAULT_PULSE_AMPLITUDE_QDAC,
                            amplitude_scale = amp_scale,
                            duration = duration,
                            validate = False
                        )

                align(*[channels[g].name for g, v in amps.items() if v !=0], readout_channel.name)
                measure('readout', readout_channel.name, I_stream, Q_stream)
            
    def pulse_sequence_builder(self, points_list:List[VoltageTuningPoint], set_offset_starting_point:bool = True, repetitions:int = 1) -> Dict[str, Dict[str, float]]:
        """Builds a dictionary of the pulse indices with their amplitudes and durations. """
        
        p1 = list(points_list)

        starting_point = self.gateset.get_voltages()
        pulse_amplitudes, pulse_durations = {}, {}
        if not p1:
            raise ValueError('No points to pulse to')
        elif set_offset_starting_point:
            V0 = p1.pop(0).voltages
            self.gateset.apply_voltages(V0, apply_cc = True)
            starting_point = V0

        #pulse_amplitudes['Pulse 1'] = {x: p1[1].voltages[x] - starting_point[x] for x in p1[1].voltages[x].keys()}
        
        for index in range(0, len(p1)):
            # amplitudes = {x: p1[index+1].voltages[x] - p1[index].voltages[x] for x in p1[index+1].voltages.keys()}
            # pulse_amplitudes[f'Pulse {index+1}'] = amplitudes
            pulse_durations[f'Pulse {index+1}'] = p1[index].duration
            pulse_amplitudes[f'Pulse {index+1}'] = {x: p1[index].voltages[x] - starting_point[x] for x in p1[index].voltages.keys()}


        qua_program = self.generate_qua_program(pulse_durations, pulse_amplitudes)
