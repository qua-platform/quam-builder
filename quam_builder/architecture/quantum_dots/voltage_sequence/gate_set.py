...
from quam.components.channels import SingleChannel, InOutSingleChannel
...
class GateSet(QuantumComponent):
    ...
    def get_voltages(self):
        current_voltages = {}
        for gate_name in self.channels.keys():
            current_voltages[gate_name] = self.channels[gate_name].get_dc_V()
...
class QdacGateSet(GateSet):
    """
    Akin to GateSet. Modifications include: 
        -Able to accept new class QdacOpxChannel, defined for both AC and DC control
        -Pass a new QuamComponent which maps the sensors to the plungers
    """
    ...
    def resolve_voltages(
        self, voltages: Dict[str, float], allow_extra_entries: bool = False, unnamed_gates_to_zero: bool = False
    ) -> Dict[str, float]:
        """
        Adds any channels in the GateSet that are not in the target_levels_dict
        to the target_levels_dict with a default voltage of 0.0.
        ...
        # Add any channels in the GateSet that are not in the voltages dict
        for ch_name in self.channels:
            if unnamed_gates_to_zero:
                resolved_voltages[ch_name] = voltages.get(ch_name, 0.0)
            else:
                resolved_voltages[ch_name] = voltages.get(ch_name, self.channels[ch_name].get_dc_V())

        return resolved_voltages

    def apply_voltages(self, voltages:Dict[str,float], allow_extra_entries:bool = False):
        """
        A function to immediately apply all the resolved DC voltages using the QDAC, not a part of QUA/OPX
        """
        resolved = self.resolve_voltages(voltages, allow_extra_entries)
        for gate_name, V in resolved.items():
            self.channels[gate_name].set_dc_V(V)
    ...
    def add_point(self, 
                    name: str, 
                    duration: int,
                    point_voltages: Dict[str, float], 
                    allow_extra_entries: bool = False, 
                    unnamed_gates_to_zero: bool = False
    ):
        invalid_channel_names = set(point_voltages) - set(self.valid_channel_names)
        if invalid_channel_names:
            raise ValueError(
                f"Channel(s) '{invalid_channel_names}' specified in voltages for point "
                f"'{name}' are not part of this GateSet."
        ...
        resolved = self.resolve_voltages(point_voltages, allow_extra_entries, unnamed_gates_to_zero)
        # Ensure macros dict exists if not handled by Pydantic model of QuantumComponent
        if not hasattr(self, "macros") or self.macros is None:
            self.macros: Dict[str, QuamMacro] = {}

        self.macros[name] = VoltageTuningPoint(voltages=resolved, duration=duration)
    
    # def new_sequence(self, track_integrated_voltage: bool = False) -> "QdacVoltageSequence":
    #     """
    #     Creates a new VoltageSequence instance associated with this GateSet.
    ...
    #     return QdacVoltageSequence(self, track_integrated_voltage)