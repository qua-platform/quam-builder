...
"""Simulate the dynamically decoupled CZ (DCZ) macro on QM SaaS.

The DCZ gate runs: CZ – X180(control, target) – CZ, refocusing low-frequency
noise while accumulating twice the exchange phase.

Example:
    python quam_builder/architecture/quantum_dots/examples/dcz_macro_example.py \
      --sim-backend cloud \
...
from quam_builder.architecture.quantum_dots.operations.voltage_balanced_macros.two_qubit_macros import (  # noqa: E402
    BalancedDCz2QMacro,
)
from quam_builder.architecture.quantum_dots.qpu import LossDiVincenzoQuam  # noqa: E402


def build_dcz_tutorial_machine() -> LossDiVincenzoQuam:
    cluster_cfg = (
        _CS_INSTALLATIONS_TESTS_CONFIG
...
def swap_qubit_pair_cz_to_dcz(machine: LossDiVincenzoQuam) -> None:
    """Replace the BalancedCz2QMacro on the first qubit pair with BalancedDCz2QMacro."""
    qubit_pair = next(iter(machine.qubit_pairs.values()))

    before_type = type(qubit_pair.macros[TwoQubitMacroName.CZ.value]).__name__
    qubit_pair.macros[TwoQubitMacroName.CZ.value] = BalancedDCz2QMacro()
    after_type = type(qubit_pair.macros[TwoQubitMacroName.CZ.value]).__name__

    print(
        f"Swapped qubit pair '{qubit_pair.name}' CZ macro: {before_type} → {after_type}",
        flush=True,
    )


def demonstrate_dcz_persistence(
    machine: LossDiVincenzoQuam,
) -> LossDiVincenzoQuam:
    """Save the machine state to disk, reload it, and verify the DCZ macro survives."""
    qubit_pair = next(iter(machine.qubit_pairs.values()))
    macro_before: BalancedDCz2QMacro = qubit_pair.macros[TwoQubitMacroName.CZ.value]

    assert isinstance(macro_before, BalancedDCz2QMacro), (
        f"Expected BalancedDCz2QMacro, got {type(macro_before).__name__}"
    )
    print(
        f"Before save  — type={type(macro_before).__name__}, "
        f"wait_duration={macro_before.wait_duration}, "
        f"ramp_duration={macro_before.ramp_duration}, "
        f"x180_pulse_name={macro_before.x180_pulse_name!r}",
        flush=True,
    )
    ...
    macro_after: BalancedDCz2QMacro = loaded_pair.macros[TwoQubitMacroName.CZ.value]

    assert isinstance(macro_after, BalancedDCz2QMacro), (
        f"Reload failed: expected BalancedDCz2QMacro, got {type(macro_after).__name__}"
    )
    assert macro_after.wait_duration == macro_before.wait_duration
    assert macro_after.ramp_duration == macro_before.ramp_duration
    assert macro_after.x180_pulse_name == macro_before.x180_pulse_name

    print(
        f"After reload — type={type(macro_after).__name__}, "
        f"wait_duration={macro_after.wait_duration}, "
        f"ramp_duration={macro_after.ramp_duration}, "
        f"x180_pulse_name={macro_after.x180_pulse_name!r}",
        flush=True,
    )
    print(
        "Persistence check passed: BalancedDCz2QMacro round-trips cleanly.",
        flush=True,
    )
    return loaded


def build_dcz_program(machine: LossDiVincenzoQuam) -> object:
    """QUA program: initialize → x90 → DCZ → measure."""
    pair = next(iter(machine.quantum_dot_pairs.values()))
    ...
    with qua.program() as program:
        pair.initialize()
        qua.align(qubit.xy.name)
        qubit.x90()
        qua.align()
        qubit_pair.cz()  # BalancedDCz2QMacro: CZ – X180(ctrl, tgt) – CZ
        qua.align()
        pair.measure()
...
def _plot_samples(
    ...
    *,
    ...
    title: str = "DCZ macro sequence (CZ – X180 – CZ)",
) -> None:
...
def run_cloud_dcz_simulation(
    ...
    *,
    ...
    swap_qubit_pair_cz_to_dcz(machine)
    machine = demonstrate_dcz_persistence(machine)
    _apply_lf_fem_output_delay(machine)
    ...
    with client.simulator(client.latest_version()) as instance:
        ...
        job = qmm.simulate(
            ...
            build_dcz_program(machine),
            SimulationConfig(duration=_DCZ_SIM_CLOCK_CYCLES),
        )
    ...
    if not no_plot:
        save_path = plot_dir / "dcz_sequence.png" if plot_dir is not None else None
        if save_path is not None:
            save_path.parent.mkdir(parents=True, exist_ok=True)
        _plot_samples(samples, machine, output_path=save_path)
    print("Simulated: DCZ macro sequence (CZ – X180 – CZ)", flush=True)
