...
def register_qubit_with_points(
    ...
    """
    # Register the qubit, linking it to a quantum dot and XY
...
def add_qubit_macros(qubit: LDQubit):
    ...
    class DriveMacro(QuamMacro):
        ...
        def apply(self, **kwargs):
            """
            Apply a microwave drive pulse to the qubit's XY.
            ...
            if parent_qubit.xy is None:
                raise ValueError(f"No XY configured for qubit {parent_qubit.id}")
        ...
        def inferred_duration(self) -> float | None:
            ...
            try:
                ...
                qd_pair_id = machine.find_quantum_dot_pair(
                    parent_qubit.quantum_dot.id,
                    parent_qubit.preferred_readout_quantum_dot,
                )
                if (
