...
4. Switch the pulse family (e.g. Gaussian -> Kaiser).
...
def wire_defaults(machine: LossDiVincenzoQuam) -> None:
    ...
    print(
        f"  q1 xy_drive pulse: {type(q1.xy.operations.get(DrivePulseName.GAUSSIAN)).__name__}"
    )
    print(
...
def update_operation_parameters(machine: LossDiVincenzoQuam) -> None:
    ...
    for qubit in machine.qubits.values():
        ...
        qubit.xy.operations[f"{DrivePulseName.GAUSSIAN}_x90"].amplitude = 0.17
        qubit.macros[SingleQubitMacroName.IDENTITY].duration = 24

    q1 = machine.qubits["q1"]
    print(
        f"  q1.initialize.ramp_duration = {q1.macros[VoltagePointName.INITIALIZE].ramp_duration}"
    )
    print(
        f"  q1.measure.buffer_duration = {q1.macros[VoltagePointName.MEASURE].buffer_duration}"
    )
    print(
        "  q1.gaussian_x90.amplitude = "
        f"{q1.xy.operations[f'{DrivePulseName.GAUSSIAN}_x90'].amplitude}"
    )
...
def update_drive_pulse_type(machine: LossDiVincenzoQuam) -> None:
    """Switch all XY-drive macros from Gaussian to Kaiser pulse family.

    The ``pulse_family`` field on each ``XYDriveMacro`` determines which
    set of operations (e.g. ``kaiser_x90``, ``kaiser_x180``) is used.
    All families are pre-wired at build time; switching is instantaneous.

    Use ``machine.set_pulse_family(family)`` to propagate the change to
    every qubit's XY macros at once.
    """
    print("\n" + "=" * 80)
    print("STEP 3: Switch pulse family (Gaussian -> Kaiser)")
    print("=" * 80)
    ...
    print("  All families (gaussian, square, kaiser) remain registered;")
    print("  macros now resolve to kaiser_* operations.")
...
def replace_macros(machine: LossDiVincenzoQuam) -> None:
    ...
    print(
        "  q1.x180 class: "
        f"{type(q1.macros[SingleQubitMacroName.X_180]).__name__} (overridden)"
    )
    print(
        "  q2.x180 class: "
        f"{type(q2.macros[SingleQubitMacroName.X_180]).__name__} (default)"
    )
    print(
        "  q1_q2.cz class: "
        f"{type(pair.macros[TwoQubitMacroName.CZ]).__name__} (overridden)"
    )
