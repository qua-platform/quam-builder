...
class StarkInducedCZGate(QubitPairMacro):
    ...
    def apply(
        ...
        def zz_shift_relative_phase():
            if zz_relative_phase is not None:
                qt.xy_detuned.frame_rotation_2pi(zz_relative_phase)
        ...
        # Pulse sequence
        zz_shift_relative_phase()
        align(zz.name, qt.xy_detuned.name)
        zz_control_drive_play()
        zz_target_drive_play()

        align(zz.name, qt.xy_detuned.name, qc.xy.name, qt.xy.name)
