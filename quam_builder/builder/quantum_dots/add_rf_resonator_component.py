...
def add_rf_resonator_component(
    qubit: Union[LDQubit], wiring_path: str, ports: Dict[str, str]
):
    """Adds a resonator component to a transmon qubit based on the provided wiring path and ports.

    Args:
        qubit (Union[LDQubit]): The qubit to which the resonator component will be added.
    ...
    if all(key in ports for key in iq_in_out_channel_ports):
        qubit.resonator = ReadoutResonatorIQ(
        ...
        RF_output_resonator = qubit.resonator.frequency_converter_up
        RF_output_resonator.channel = qubit.resonator.get_reference()
        RF_output_resonator.output_mode = "always_on"

        RF_input_resonator = qubit.resonator.frequency_converter_down
        RF_input_resonator.channel = qubit.resonator.get_reference()
        ...
        qubit.resonator = ReadoutResonatorMW(
        ...
        qubit.resonator.opx_input.downconverter_frequency = (
            f"{qubit.resonator.opx_output.get_reference()}/upconverter_frequency"
        )
