...
class LineTypeRegistry:
    ...
    def _setup_default_mappings(self):
        ...
        Optional line types (quantum dots, TWPA) are registered conditionally.
        """
        ...
        if hasattr(WiringLineType, "PLUNGER_GATE"):
            qubit_lines.append(WiringLineType.PLUNGER_GATE)
        ...
        if hasattr(WiringLineType, "BARRIER_GATE"):
            pair_lines.append(WiringLineType.BARRIER_GATE)
        ...
        if hasattr(WiringLineType, "GLOBAL_GATE"):
            self.register(WiringLineType.GLOBAL_GATE, ElementCategory.GLOBAL_ELEMENT)

        # Readout line types - measurement operations
        if hasattr(WiringLineType, "SENSOR_GATE"):
            self.register(WiringLineType.SENSOR_GATE, ElementCategory.READOUT)
        if hasattr(WiringLineType, "RF_RESONATOR"):
            self.register(WiringLineType.RF_RESONATOR, ElementCategory.READOUT)

        # TWPA line types - parametric amplifier pump lines
        if hasattr(WiringLineType, "PUMP"):
            self.register(WiringLineType.PUMP, ElementCategory.TWPA)
