...
try:
    from qualang_tools.wirer.connectivity.element import TWPAReference
except ImportError:
    TWPAReference = None
...
def _create_wiring_legacy(connectivity: Connectivity) -> dict:
    ...
    for element_id, element in connectivity.elements.items():
        for line_type, channels in element.channels.items():
            ...
            elif hasattr(WiringLineType, "PUMP") and line_type_value == WiringLineType.PUMP.value:
...
def twpa_wiring(
    ...
    """Generate QUAM JSON references for a TWPA's channels with a single line type.

    Args:
        channels (List[AnyInstrumentChannel]): The list of instrument channels.
        element_id: The ID of the TWPA element.
        line_type (WiringLineType): The type of wiring line.
