    ...
    for element_id, element in connectivity.elements.items():
        for line_type, channels in element.channels.items():
            ...
            twpa_line_values = {
                ...
                WiringLineType.TWPA_ISOLATION.value,
            }
            is_twpa_element = str(element_id).lower().startswith("twpa")

            if line_type_value in qubit_line_values or (
                line_type_value in twpa_line_values and not is_twpa_element
            ):
            ...
            elif line_type_value in twpa_line_values and is_twpa_element:
                for k, v in twpa_wiring(channels).items():
                    set_nested_value_with_path(
                        wiring, f"twpas/{element_id}/{line_type_value}/{k}", v
                    )
            else:
...
def twpa_wiring(channels: List[AnyInstrumentChannel]) -> dict:
    """Build wiring entries for TWPA pump/isolation lines."""
    twpa_line_wiring = {}
    for channel in channels:
        if not (channel.signal_type == "digital" and channel.io_type == "input"):
            key, reference = get_channel_port(channel, channels)
