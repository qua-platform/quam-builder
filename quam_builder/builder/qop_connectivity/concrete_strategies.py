...
class TwpaWiringStrategy(WiringStrategy):
    ...
    Handles wiring for TWPA pump lines used in parametric amplification.
    """

    def get_base_path(self, context: WiringContext) -> str:
        """Get base path for TWPA wiring.

        Format: twpas/{element_id}/{line_type}
        ...
        Returns:
            Base path string
        """
        return f"twpas/{context.element_id}/{context.line_type.value}"

    def get_additional_references(self, context: WiringContext) -> Dict[str, str]:
        """Get additional references for TWPA elements.

        TWPAs don't need additional references beyond their port mappings.
